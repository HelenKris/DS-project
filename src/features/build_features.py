...
class FeatureEngineering:
    ...
    def _get_price_dynamics(self, data):
        ...
        rename_dict = {col: f'Price {i + 1}' for i, col in enumerate([col for col in price_dynamics.columns][::-1])}
    ...
    def _get_mean_features(self, data):
        ...
        group_means_shop_item = df_filtered.groupby(['shop_id', 'item_id'])[x_col_names].mean().mean(axis=1).reset_index(name='shop_item_cnt_mean')
        group_means_shop_item_category = df_filtered.groupby(['shop_id', 'item_category_name'])[x_col_names].mean().mean(axis=1).reset_index(name='shop_item_category_id_cnt_mean')
        group_means_sity_item_id = df_filtered.groupby(['shop_name', 'item_id'])[x_col_names].mean().mean(axis=1).reset_index(name='sity_item_id_cnt_mean')
        group_means_shop_item_category = df_filtered.groupby(['shop_id', 'item_category_name'])[x_col_names].mean().mean(axis=1).reset_index(name='shop_item_category_cnt_mean')
        group_means_sity_category_id_cnt_mean = df_filtered.groupby(['shop_name', 'item_category_id'])[x_col_names].mean().mean(axis=1).reset_index(name='sity_category_id_cnt_mean')
        group_means_sity_item_category = df_filtered.groupby(['shop_name', 'item_category_name'])[x_col_names].mean().mean(axis=1).reset_index(name='sity_item_category')
    ...
    def _get_other_features(self, data):
        ...
        category_means = data.groupby('item_category_id')['Price 1'].mean().reset_index()
        bins = [0, 100, 500, 1000, 2000, 3000, 4000, 5000, 10000, 23000]
        category_means['cat_price_cat'] = pd.cut(category_means['Price 1'], bins=bins, labels=[1, 2, 3, 4, 5, 6, 7, 8, 9])
        data = pd.merge(data, category_means.drop(labels=['Price 1'], axis=1), on=['item_category_id'], how='left')

        group_means_shop_item_cat_cnt_cat = data.groupby(['shop_id', 'cat_price_cat'])[x_col_names].mean().mean(axis=1).reset_index(name='shop_item_cat_cnt_cat')
        data = data.merge(group_means_shop_item_cat_cnt_cat, on=['shop_id', 'cat_price_cat'], how='left')
        group_means_sity_item_cat_cnt_cat = data.groupby(['shop_name', 'cat_price_cat'])[x_col_names].mean().mean(axis=1).reset_index(name='sity_item_cat_cnt_cat')
        data = data.merge(group_means_sity_item_cat_cnt_cat, on=['shop_name', 'cat_price_cat'], how='left')
